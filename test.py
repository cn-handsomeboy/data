...
def test_cross_border_consistency():
    ...
    for country in countries:
        ...
        policy_text = (
            ...
            f"与中国-广西（{system._fao_yield基准.get('China', 5.96):.2f}吨/亩）相比"
            f"仍有产量提升空间。"
        )
    ...
    # 14.5 验证FAO基准数据完整性
    for country in countries:
        assert country in system._fao_yield基准, f"{country}应在FAO基准中"
        assert system._fao_yield基准[country] > 0, f"{country}FAO基准应大于0"
    print(f"[PASS] 14.5 FAO基准完整性: {', '.join([f'{country_cn[c]}={system._fao_yield基准[c]:.2f}' for c in countries])}")
